...
load("power_sim_disjsyll_order_as_factor_1000iter.RData")
```
...
## add order
start.data$order <- as.factor(rep(x = c("test_first", "control_first"), times = nrow(start.data) / 2))
##roger: this last line is a little 'dangerous' as it relies on the fact that start.data comprises individuals in the form
	##1, 2, 3... and this repeated multiple times
	##a safer way is:
	as.factor(rep(x = c("test_first", "control_first"), times = n.subject / 2))[as.numeric(start.data$subj.id)]
...
##roger: this last line seems unnecessarily complicated to me
	##you could also use
...
```{r include=FALSE}
#not relevant for the simulation, chunk can be removed.
...
plot(x=as.factor(pv), y=rv, type="hist", ylim=c(0, 1),
...
for (i in 1:nrow(all.res)) {
  ...
  # store results:
  all.res[i, c("icpt", "conditiontest", "z.age", "sexM", "z.trial", "ordertest_first")] <- fixef(full$value)
...
#save.image("power_sim_disjsyll_order_as_factor_1000iter.RData")
```
...
ggplot(data = lrt.data2, aes(y=proportion.sign.lrt2))+
  geom_point(aes(x=as.factor(r.effect), y=proportion.sign.lrt2))+
  geom_boxplot(aes(x=as.factor(r.effect), y=proportion.sign.lrt2, group=r.effect))+
  #theme_classic()+
  ylim(0.3, 1)+
  geom_hline(yintercept=0.8, colour="red", lty=2)+
  facet_wrap(~r.slope.con)
...
```{r echo=FALSE}
ggplot(data = all.res2, aes(x=as.factor(r.effect), y=conditiontest))+
  geom_jitter( alpha=0.5, col="grey")+
  geom_boxplot(aes(x=as.factor(r.effect), y=conditiontest, group=r.effect), alpha=0.1, outlier.colour="white")+
  geom_hline(yintercept=coefs["conditiontest"], colour="red", lty=2)+
  facet_wrap(~r.slope.con)
```
...
```{r eval=FALSE, include=FALSE}
#save.image("power_sim_disjunctive_syll_order_as_factor.RData")
```
