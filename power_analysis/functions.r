...
##all were written by Roger Mundry
##function replacing drop1; the reasons for using it rather than drop1 is that it allows to hand over a control object
	##and that it returns information about failure to converge about individual models
	...
	if(length(x.all.res)>0){
		all.tests=lapply(1:length(x.all.res), function(x){
			...
			return(list(logLik=as.vector(logLik(x.all.res[[x]])), AIC=extractAIC(x.all.res[[x]])[2], Chisq=as.vector(ires["Chisq"]), "Chi Df"=as.vector(ires["Chi Df"]), 
				...
				n.opt.warnings=paste(unlist(x.all.res[[x]]@optinfo$conv$lme4), collapse=""), n.fun.warnings=paste(unlist(x.all.res[[x]]@optinfo$warnings), collapse="")))
		})
		...
		all.tests[, "n.opt.warnings"]=as.character(all.tests[, "n.opt.warnings"])
		all.tests[, "n.fun.warnings"]=as.character(all.tests[, "n.fun.warnings"])
		all.tests[is.na(all.tests[, "n.opt.warnings"]), "n.opt.warnings"]=""
		all.tests[is.na(all.tests[, "n.fun.warnings"]), "n.fun.warnings"]=""
...
##function extracting warnings from models fitted with lme4 functions:
...
##function determine the correction for for small samples to be added to an AIC
