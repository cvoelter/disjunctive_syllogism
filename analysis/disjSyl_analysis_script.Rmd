...
exp12_plot <- ggplot(data = xdata2.agg, aes(x = experiment, y = mean.resp)) +
  ...
  scale_x_continuous(breaks = c(0.75, 2.25), labels = c("Two-Cup Task\n", "Three-Cup Task\n"), limits = c(0,3)) +
  ylim(0, 1) +
  theme_classic()
...
xx_exp3_plot <- as.data.frame(round(boot.res_exp3.plot$ci.estimates, 3))
xx_exp3_plot
...
exp3_plot <- ggplot(data = xdata3.agg, aes(x = condition, y = mean.resp)) +
  ...
  xlab("") +
  ylab("Proportion of target-cup choices") +
  scale_x_continuous(breaks = c(0.75, 2.25), labels = c("Reveal Empty Cup\n Control", "Reveal Empty Cup\n Test"), limits = c(0,3)) +
  ylim(0, 1) +
  theme_classic()
...
chance.test_exp3.t <- glmer(correct ~ 1 +
  (1 + z.trial | subj.id),
data = xdata3%>%filter(condition == "test"), family = binomial, control = contr
)

summary(chance.test_exp3.t)$coef

#we subtracted the logit-transformed chance-value from the estimate of the intercept and calculated the corresponding z- and p-value based on this adjusted estimate.
z=(summary(chance.test_exp3.t)$coef[1,1]-(log((1/3)/(1-(1/3)))))/(summary(chance.test_exp3.t)$coef[1,2])
round(z, 2)

# corrected chance value
2*pnorm(-(abs(z)))

#corrected estimate
summary(chance.test_exp3.t)$coef[1,1]-(log((1/3)/(1-(1/3))))
...
chance.test_exp3.c <- glmer(correct ~ 1 +
  (1 + z.trial | subj.id),
data = xdata3%>%filter(condition == "control"), family = binomial, control = contr
)

summary(chance.test_exp3.c)$coef

#we subtracted the logit-transformed chance-value from the estimate of the intercept and calculated the corresponding z- and p-value based on this adjusted estimate.
z.c=(summary(chance.test_exp3.c)$coef[1,1]-(log((1/3)/(1-(1/3)))))/(summary(chance.test_exp3.c)$coef[1,2])
round(z.c, 2)
...
#corrected estimate
summary(chance.test_exp3.c)$coef[1,1]-(log((1/3)/(1-(1/3))))
...
xx_exp4_plot <- as.data.frame(round(boot.res_exp4.plot$ci.estimates, 3))
xx_exp4_plot
...
exp4_plot <- ggplot(data = xdata4.agg, aes(x = condition, y = mean.resp)) +
  ...
  scale_x_continuous(breaks = c(0.75, 2.25), labels = c("Reveal Baited Cup\n Control", "Reveal Baited Cup\n Test"), limits = c(0,3)) +
  ylim(0, 1) +
  theme_classic()
...
xx_exp3_exp4_plot <- as.data.frame(round(boot.res_exp3_exp4.plot$ci.estimates, 3))
xx_exp3_exp4_plot
